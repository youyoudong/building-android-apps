...
Build scans generate a complete report of all the dependencies, plugins, test outputs, and performance of your build.
You can easily enable build scans in your build by following these steps:

1. If you're using Gradle 4.3+, run your builds with the `--scan` command line option, e.g. `gradle build --scan`
1. If you're using Gradle < 4.3, manually apply the https://docs.gradle.com/scans[build scan plugin], then run your builds with the `--scan` command line option

By doing one of the above options, you are going to publish a build scan of the Gradle build that was just executed to https://scans.gradle.com.
This requires accepting a license agreement, and will be asked during the build (Gradle 4.3+) or can be accepted upfront (any Gradle version).
See the https://docs.gradle.com/build-scan-plugin/[Build Scan Plugin User Manual] for more explanations or configuration possibilities.

[source,groovy]
.Example Gradle build, when manually applying the build scan plugin
----
buildscript {
    repositories {
        jcenter()
    ...
    dependencies {
        classpath 'com.android.tools.build:gradle:2.4.0-alpha7'
        classpath 'com.gradle:build-scan-plugin:1.10.3'  // <1>
    }
}
...
When you run `./gradlew build --scan`, you will receive a link to the results, hosted at Gradle.
----
./gradlew build --scan
